...
2. List the hubs returned, with the user's role in each. A role below `content_creator` means every other call will come back "not found"; say that explicitly. A role below `admin` means the metrics tools will too, even when they are present — call that out separately.
