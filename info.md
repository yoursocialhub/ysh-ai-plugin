...
Beyond scopes, every call is checked against your hub membership: you need the `content_creator` or `admin` role in the hub that owns the data. A reviewer-only account can connect but will reach nothing.
...
- **Touch social accounts.** Connections, tokens and metrics are outside its reach entirely.
