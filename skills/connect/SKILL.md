...
Three consequences worth saying out loud before they click:
...
Call `list_hubs`. Then list which of the 13 tools are actually present and tell the user plainly which workflows that leaves them:

- no `hubs:read` **or** no `content-calendars:read` → nothing works, whatever else was ticked;
- all four reads → `calendar-audit` works in full; `plan-week`, `draft-captions` and `promote-to-review` do not;
